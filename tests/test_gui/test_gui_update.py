...
def update_harness(
	...
	def _create_harness(
		...
		monkeypatch.setattr(
			"gui.auto_updater.check_dependencies",
			lambda *args, **kwargs: (missing_deps, outdated_deps)
		)

		# Mock user choice
		monkeypatch.setattr("gui.auto_updater.install_prompt", lambda *args: user_response)
		...
		monkeypatch.setattr("gui.auto_updater.get_modules_for_package", lambda *args: ["mock_module"])
