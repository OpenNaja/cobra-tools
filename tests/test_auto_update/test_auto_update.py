...
from gui.auto_updater import check_dependencies, extract_package_name, get_modules_for_package
...
def test_pip_install_builds_correct_command(monkeypatch):
	...
	from gui.auto_updater import pip_install
...
def test_pip_upgrade_builds_correct_command(monkeypatch):
	...
	# Import and run the function to be tested
	from gui.auto_updater import pip_upgrade
...
def test_run_update_check_orchestration(monkeypatch):
	...
	monkeypatch.setattr("gui.auto_updater.check_dependencies", lambda *args: (missing_deps, outdated_deps))
	monkeypatch.setattr("gui.auto_updater.get_modules_for_package", lambda *args: [])
	monkeypatch.setattr("tomllib.load", lambda *args: MOCK_PYPROJECT_DATA)

	# Mock user input
	monkeypatch.setattr("gui.auto_updater.install_prompt", lambda *args: True)
	...
	def spy_pip_upgrade(packages: list[str]):
		...
		return 0
	monkeypatch.setattr("gui.auto_updater.pip_install", spy_pip_install)
	monkeypatch.setattr("gui.auto_updater.pip_upgrade", spy_pip_upgrade)
	# Patch out the restart logic so the test doesn't exit.
	monkeypatch.setattr("gui.auto_updater._relaunch_application", lambda: None)
	# Patch file I/O and run the main function
	with patch("gui.auto_updater.open", MagicMock()):
...
def test_get_all_deps(tool_name, expected_tool_specific_deps):
	...
	"""
	from gui.auto_updater import get_all_deps
