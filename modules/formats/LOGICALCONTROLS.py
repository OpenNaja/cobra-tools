...
class LogicalControlsLoader(BaseFile):
    ...
    def create(self):
        ...
        for lcdata in lclistdata:
            ...
            for arg in argxmldata:
                argdata = {'k1a': int(arg.attrib['k1a']), 'k1b': int(arg.attrib['k1b']), 'k2': int(arg.attrib['k2']),
                           'k3': int(arg.attrib['k3']), 'k4': int(arg.attrib['k4'])}
    ...
    def collect(self):
        ...
        _, _, _, _, c1, c2, c3, c4, flags, _ = struct.unpack("<4Q 4B I Q", self.sized_str_entry.pointers[0].data)
        logicalcontrols = []
        ...
        if c1 > 0:
            ...
            for x in range(c1):
                ...
                name = self.p1_ztsr(namefragment)
        ...
        logging.debug(f"Captured {len(logicalcontrols)} controls")
