...
def dat_hasher(ovl, name_tups):
	...
	# ovl_lists = [ovl.files, ovl.textures, ]
	ovl_lists = [ovl.files, ]
	ovs_lists = []
	...
	for entry_list in ovl_lists:
		for entry in entry_list:
			try:
				...
				old_hash_to_new[entry.file_hash] = (new_name, new_ext, new_hash)
				print(f"{entry.name} -> {new_name},  {entry.file_hash} ->  {new_hash}")
	...
	for i, entry_list in enumerate(ovs_lists):
		for entry in entry_list:
			new_name, new_ext, new_hash = old_hash_to_new[entry.file_hash]
			entry.file_hash = new_hash
			entry.name = f"{new_name}.{new_ext}"
