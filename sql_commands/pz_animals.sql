...
-- NOT FOR MANUAL USE
-- Please see #pz-modding-resources in the modding discord for manual scripts
...
-- 1.15 Tables
CREATE TABLE IF NOT EXISTS ViewingDomeParticipantData (AnimalType TEXT REFERENCES AnimalDefinitions (AnimalType) ON UPDATE CASCADE NOT NULL COLLATE NOCASE, DoActionAtDomeWeight DOUBLE CHECK (DoActionAtDomeWeight <= 1.0 AND DoActionAtDomeWeight >= 0.0) NOT NULL DEFAULT (0.0), GoToDomeWeight DOUBLE CHECK (GoToDomeWeight <= 1.0 AND GoToDomeWeight >= 0.0) NOT NULL DEFAULT (1.0));
...
DELETE FROM ViewingDomeParticipantData WHERE AnimalType NOT LIKE ('%'||(SELECT Original FROM Replacement)||'%') AND AnimalType NOT LIKE ('%'||(SELECT New FROM Replacement)||'%');
...
UPDATE ViewingDomeParticipantData SET AnimalType = (SELECT AnimalType FROM AnimalDefinitions WHERE AnimalType LIKE '%'||(SELECT New FROM Replacement)||'%') WHERE AnimalType LIKE '%'|| (SELECT Original FROM Replacement) ||'%';
