...
def generate_vertex_remap(destination: np.ndarray,
    ...
    """
    # Evaluate vertices first so vertex_count is available as a fallback for unindexed geometry
    ...
    if indices is not None:
        ...
        # For unindexed geometry, the C++ backend iterates up to index_count.
        # It must match the number of vertices being evaluated.
        if index_count is None:
            index_count = vertex_count if vertex_count is not None else 0
