from generated.base_struct import BaseStruct
from generated.formats.base.basic import Ushort


class UshortVector2(BaseStruct):

	"""
	A vector in 2D space (x,y).
	"""

	__name__ = 'UshortVector2'

	_import_path = 'generated.formats.manis.compounds.UshortVector2'

	def __init__(self, context, arg=0, template=None, set_default=True):
		super().__init__(context, arg, template, set_default=False)

		# First coordinate.
		self.u = 0

		# Second coordinate.
		self.v = 0
		if set_default:
			self.set_defaults()

	def set_defaults(self):
		super().set_defaults()
		self.u = 0
		self.v = 0

	@classmethod
	def read_fields(cls, stream, instance):
		super().read_fields(stream, instance)
		instance.u = Ushort.from_stream(stream, instance.context, 0, None)
		instance.v = Ushort.from_stream(stream, instance.context, 0, None)

	@classmethod
	def write_fields(cls, stream, instance):
		super().write_fields(stream, instance)
		Ushort.to_stream(stream, instance.u)
		Ushort.to_stream(stream, instance.v)

	@classmethod
	def _get_filtered_attribute_list(cls, instance):
		yield from super()._get_filtered_attribute_list(instance)
		yield 'u', Ushort, (0, None), (False, None)
		yield 'v', Ushort, (0, None), (False, None)

	def get_info_str(self, indent=0):
		return f'UshortVector2 [Size: {self.io_size}, Address: {self.io_start}] {self.name}'

	def get_fields_str(self, indent=0):
		s = ''
		s += super().get_fields_str()
		s += f'\n	* u = {self.fmt_member(self.u, indent+1)}'
		s += f'\n	* v = {self.fmt_member(self.v, indent+1)}'
		return s

	def __repr__(self, indent=0):
		s = self.get_info_str(indent)
		s += self.get_fields_str(indent)
		s += '\n'
		return s
