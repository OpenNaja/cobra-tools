...
class BnkBufferData:
	...
	def __init__(self, context, arg=0, template=None, set_default=True):
		...
		# 0
		self.zeros_old = numpy.zeros((4,), dtype=numpy.dtype('uint32'))
		...
		# ext format subtypes
		self.extensions = Array((self.suffix_count,), ZString, self.context, 0, None)
		if set_default:
			self.set_defaults()
	...
	def set_defaults(self):
		...
		self.stream_info_count = 0
		self.zeros = numpy.zeros((7,), dtype=numpy.dtype('uint32'))
		if (self.context.user_version.is_jwe and (self.context.version == 19)) or (self.context.version == 18):
			self.zeros_old = numpy.zeros((4,), dtype=numpy.dtype('uint32'))
		self.stream_infos = Array((self.stream_info_count,), StreamInfo, self.context, 0, None)
		self.name = ''
		self.extensions = Array((self.suffix_count,), ZString, self.context, 0, None)
	...
	def read_fields(cls, stream, instance):
		...
		if (instance.context.user_version.is_jwe and (instance.context.version == 19)) or (instance.context.version == 18):
			instance.zeros_old = stream.read_uints((4,))
		...
		instance.extensions = stream.read_zstrings((instance.suffix_count,))
	...
	def write_fields(cls, stream, instance):
		...
		if (instance.context.user_version.is_jwe and (instance.context.version == 19)) or (instance.context.version == 18):
			stream.write_uints(instance.zeros_old)
	...
	def get_fields_str(self, indent=0):
		...
		s += f'\n	* zeros_old = {fmt_member(self.zeros_old, indent+1)}'
